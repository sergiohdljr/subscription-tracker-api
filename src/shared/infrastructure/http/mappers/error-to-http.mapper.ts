...
import {
  InvalidBillingDateError,
  InvalidTrialPeriodError,
  InvalidSubscriptionNameError,
} from '@/modules/subscriptions/domain/errors';
...
export function mapDomainErrorToHttp(error: Error, request?: FastifyRequest): HttpError {
  ...
  if (
    error instanceof InvalidBillingDateError ||
...
export function formatErrorResponse(httpError: HttpError, request?: FastifyRequest): ErrorResponse {
  return {
